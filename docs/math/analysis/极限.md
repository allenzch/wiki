...
基给出了一种广义的极限测量方式，即测量条件不断精确的近似测量，基就是一系列不断精确的测量条件。这种情况下，近似值集合并不一定收缩，也并不一定唯一确定一个值。以上定义中的集合 $X$ 一般是所有可能或同类测量条件的集合。

### 基上的极限

设 $f : X \to Y$ 是集合 $X$ 上的函数，$\mathcal{B}$ 是 $X$ 中的集。如果对于点 $A \in Y$ 的任意一个邻域 $V(A)$，可以找到基中的元素 $B \in \mathcal{B}$，使得该元素的像 $f(B)$ 包含于邻域 $V(A)$，则称 $A$ 为函数 $f : X \to Y$ 在基 $\mathcal{B}$ 上的极限。

其中，集合 $Y$ 上定义了度量或拓扑结构，而邻域 $V(A)$ 有度量上的和拓扑上的两种定义方式，可以证明，当拓扑由度量诱导时，两种定义给出的极限是等价的. 近似得到测量值通过函数来数学化. 定义中，通过函数 $f : X \to Y$ 将测量中由近似得到测量值的过程数学化，$f(B)$ 即为在一测量条件下的测量值范围. 定义中的任意邻域 $V(A)$ 隐含了不断缩小靠近 $A$ 的过程，即对于给定基 $\mathcal{B}$ 和函数 $f$（一个测量），若其满足上述条件，其所得的测量值范围不断缩小靠近 $A$. 然而，即使在这种情况下，该测量仍不能唯一确定值 $A$.
...
如果随着测量条件的逐渐精确，测量出的近似也足够精确，那么这种测量得到的值就是极限。
