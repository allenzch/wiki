...
$\mathbb{R}$ 上拓扑取 $d$ 的诱导拓扑 $\mathscr{T}$，则 $\mathscr{T}$ 是 $\mathbb{R}$ 上的通常拓扑.
...
子集 $\{0, 1, 2, \cdots\}$ 满足皮亚诺公理，即为**自然数**，记作 $\mathbb{N}$. 通过 $\mathbb{N}$ 构造可以得到整数集 $\mathbb{Z}$ 和 有理数集 $\mathbb{Q}$。

## 实函数与极限

**定义.** 映射 $f : X \to \mathbb{R}$ 称为**实函数**，简称函数，其中 $X \subset \mathbb{R}$. 实函数即为实分析的主要研究对象.
...
**定理.** 记 $A = \lim_{\mathcal{B}}f(x),\ B = \lim_{\mathcal{B}}g(x)$，则 $\lim_{\mathcal{B}}(f + g)(x) = A + B,\ \lim_{\mathcal{B}}(f \cdot g)(x) = AB$，此外，如果 $B \ne 0$，则 $\lim_{\mathcal{B}}\frac{f}{g} = \frac{A}{B}$.

可以证明 $\lim_\mathcal{B}C = C$，其中 $\mathcal{B}$ 是任意 $\mathbb{R}$ 中的基.
...
**定义.** 函数 $f : X \to \mathbb{R}$ 称为基 $\mathcal{B}$ 上相对于函数 $g : Y \to \mathbb{R}$ 的无穷小，如果 $\exists B \in \mathcal{B} \ B \subset X \cap Y \ne \varnothing$，且存在函数 $\alpha : B \to \mathbb{R}$ 满足 $\lim_{\mathcal{B}}\alpha(x) = 0$，且 $\forall x \in B \ f(x) = \alpha(x)g(x)$，记作 $f =_{\mathcal{B}} o(g)$.
...
**定义.** 函数 $f : X \to \mathbb{R}$ 称为在点 $a$ 上连续，如果 $\lim_{X \ni x \to a}f(x) = f(a)$. $f$ 称为在集合 $A$ 上连续，如果 $\forall a \in A \ \lim_{X \ni x \to a} f(x) = f(a)$.

:::note
可以证明 $f$ 在 $a$ 点处连续的必要条件是 $a \in X \cap X'$
:::

### 可微

**定义.** 函数 $f : X \to \mathbb{R}$ 称为在点 $a$ 上可微，如果 $f(x) - f(a) - \lambda(x - a) =_{X \ni x \to a} o(x - a)$. $f$ 称为在集合 $A$ 上可微，如果 $\exists \lambda \in \mathbb{R} \ \forall a \in A \ f(x) -f(a) - \lambda(x - a) =_{X \ni x \to a} o(x - a)$. 可以证明，$\lambda$ 唯一，$\lambda = \lim_{X \ni x \to a} \frac{f(x) - f(a)}{x - a}$，称为 $f$ 在 $a$ 处的导数，记作 $f'(a)$.

:::note
可以证明如果 $f'(a)$ 存在，则 $f$ 在 $a$ 上可微.

可以证明如果函数 $f$ 在点 $a$ 上可微，则  
$f$ 在 $a$ 上连续.  
$f|_A$ 在 $a$ 上可微，其中 $a \in A \cap A'$
:::

**定理.** 如果函数 $f : X \to \mathbb{R},\ g : Y \to \mathbb{R}$ 在点 $a$ 上可微，则 $f + g$ 在 $a$ 上可微.
